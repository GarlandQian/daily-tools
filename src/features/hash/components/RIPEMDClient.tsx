...
export default function RIPEMDClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const apiResult = await lookupHash('ripemd160', target)
    if (apiResult) {
            ...
            <div className="space-y-2">
              ...
              >
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="encrypt" id="ripemd-encrypt" />
                  <Label htmlFor="ripemd-encrypt" className="cursor-pointer">
                    {t('app.hash.generate')}
                  </Label>
                </div>
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="verify" id="ripemd-verify" />
                  <Label htmlFor="ripemd-verify" className="cursor-pointer">
                    {t('app.hash.verify')}
                  </Label>
                </div>
              </RadioGroup>
