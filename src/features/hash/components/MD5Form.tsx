...
import { lookupHash } from '../utils/lookup'
...
export default function MD5Form() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const found = commonPasswords.find(pwd => CryptoJS.MD5(pwd).toString().toLowerCase() === target)
    ...
    const apiResult = await lookupHash('md5', target)
    if (apiResult) {
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="encrypt" id="md5-encrypt" />
                  <Label htmlFor="md5-encrypt" className="cursor-pointer">
                    {t('app.hash.generate')}
                  </Label>
                </div>
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="verify" id="md5-verify" />
                  <Label htmlFor="md5-verify" className="cursor-pointer">
                    {t('app.hash.verify')}
                  </Label>
                </div>
              </RadioGroup>
