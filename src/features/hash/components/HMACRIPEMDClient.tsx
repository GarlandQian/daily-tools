...
export default function HMACRIPEMDClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'encrypt') {
      ...
      const hash = CryptoJS.HmacRIPEMD160(message, key).toString()
      setResult({ text: hash })
      return
    }
    ...
    const found = commonPasswords.find(
      pwd => CryptoJS.HmacRIPEMD160(pwd, key).toString().toLowerCase() === target
    )
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="encrypt" id="hmacripemd-encrypt" />
                  <Label htmlFor="hmacripemd-encrypt" className="cursor-pointer">
                    {t('app.hash.generate')}
                  </Label>
                </div>
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="verify" id="hmacripemd-verify" />
                  <Label htmlFor="hmacripemd-verify" className="cursor-pointer">
                    {t('app.hash.verify')}
                  </Label>
                </div>
              </RadioGroup>
