...
export default function NetworkWaterfallClient() {
  ...
  const outputPreviewParsed = useMemo<ParsedWorkspace>(
    () => ({
      ...
      requests: parsed.requests.slice(0, OUTPUT_PREVIEW_ROWS)
    }),
    [parsed.errors, parsed.requests]
  )
  ...
  const outputPreviewVisibleRows =
    (outputPreviewUsesParsedRows ? outputPreviewParsed.requests.length : 0) +
    (outputPreviewUsesFindings ? outputPreviewFindings.length : 0)
  const outputPreviewTotalRows =
    (outputPreviewUsesParsedRows ? parsed.requests.length : 0) +
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                ...
                variant="outline"
                onClick={() =>
                  downloadText(buildCurrentCsv(), 'network-waterfall.csv', 'text/csv;charset=utf-8')
                }
                className="w-full sm:w-auto"
              >
                <Download className="h-4 w-4" />
        ...
        <Card className="glass-panel glass-clip">
          ...
          <CardContent className="space-y-4">
            <div className="relative">
              ...
              <Input
                value={requestQuery}
                onChange={event => setRequestQuery(event.target.value.slice(0, 160))}
