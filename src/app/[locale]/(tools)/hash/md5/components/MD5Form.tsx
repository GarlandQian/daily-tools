...
export default function MD5Form() {
  ...
  const { addHistory, lookupHistory } = useHistory('md5')

  const onFinish = async (values: MD5Params) => {
    if (values.mode === 'encrypt' && values.message) {
      const hash = CryptoJS.MD5(values.message).toString()
      setResult({ text: hash })
      addHistory({
      ...
      const found = commonPasswords.find(
        (pwd) => CryptoJS.MD5(pwd).toString().toLowerCase() === target
      )
