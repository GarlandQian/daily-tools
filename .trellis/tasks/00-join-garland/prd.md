...
`garland` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/garland/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/garland/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee garland` to
...
python3 ./.trellis/scripts/task.py archive 00-join-garland
```
